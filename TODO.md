...
- **Out-of-place trailing talks in quest `74078` ("溪舟的尾波").** After the earliest-reference ordering fix, three talks still land at the very end (orders 100/103/105) but read as mid-quest content: order 100 = talk `7407812` ("卡特皮拉: 行，我也送你们一程吧。"), order 103 = talk `7407801` ("派蒙: 纳齐森科鲁兹，你从哪里看来的这些东西？"), order 105 = talk `7407802` ("纳齐森科鲁兹: 「伊啊，伊啊，潘…」"). Suspicious signal: talks `7407801`/`7407802` carry the LOWEST subId increments (likely authored early) yet their only finishCond reference is a high-`order` `COMPLETE_TALK` (subIds `7407803`/`7407805`), so they get gated to the end. Investigate whether their true play point is earlier and only encoded outside `finishCond` (Lua trigger, talk-internal condition, or a branch), and whether subId-increment order should be a tiebreaker/fallback ordering signal when finishCond placement looks late. Likely related to the subQuest-DAG investigation below.

- **Map the quest subQuest DAG / branching (vs. flat `order`).** We currently render quest talks in a single linear sequence keyed on the subQuest `order` field, but real quests can branch or run steps in parallel, so `order` may not reflect true narrative structure. Example to dig into: quest `72234` ("灯上的文字" — the lantern-text quest), whose orders 4–6 are three separate FINISH_PLOT readables (lantern messages: "我明明认得路的，地走官一定要说我走丢了…", "妈妈最近心情好转了一些…", "陆鳗鳗鱼苗饭真好吃…") that read like parallel, any-order collectibles rather than a strict sequence. Investigate whether the subQuest data encodes an actual dependency graph (candidate signals: per-subQuest accept/fail conditions among the still-obfuscated subQuest fields, `descTextMapHash` objective text, talk `nextDialogs` chains) or whether the true control flow only lives in the quest's Lua scripts (`BinOutput/LuaPath`/quest scripts). Goal: decide whether to model branches explicitly or keep the linear approximation, and whether parallel steps should be grouped rather than ordered. Decoding new subQuest fields → use the `agd-deobfuscate` skill + the older-cleartext-version trick in `AGENTS.md` > AGD Deobfuscation.
