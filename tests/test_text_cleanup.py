...
def test_nickname_replacement_english():
    ...
    result = text_cleanup.clean_text_markers(text, "ENG")
...
def test_nickname_replacement_chinese():
    ...
    result = text_cleanup.clean_text_markers(text, "CHS")
    assert result == "你好旅行者，你好吗？"


def test_nickname_replacement_fallback():
    """Test {NICKNAME} replacement with unknown language falls back to English."""
    text = "你好{NICKNAME}，你好吗？"
    result = text_cleanup.clean_text_markers(text, "FR")
    assert result == "你好Traveler，你好吗？"
...
def test_empty_text():
    ...
    result = text_cleanup.clean_text_markers("", "ENG")
    assert result == ""


def test_none_text():
    """Test None text handling."""
    result = text_cleanup.clean_text_markers(None, "ENG")
