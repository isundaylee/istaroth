...
ssh -p $REMOTE_PORT root@$REMOTE_HOST "source \$HOME/.local/bin/env 2>/dev/null; command -v uv >/dev/null || (curl -LsSf https://astral.sh/uv/install.sh | sh)"
ssh -p $REMOTE_PORT root@$REMOTE_HOST "source \$HOME/.local/bin/env && cd $WORK_ROOT/istaroth && uv sync --no-default-groups --group ml"
...
ssh -p $REMOTE_PORT root@$REMOTE_HOST "source \$HOME/.local/bin/env && cd $WORK_ROOT/istaroth && uv run python3 scripts/rag_tools.py build $DATA_ROOT/text $DATA_ROOT/checkpoint"
