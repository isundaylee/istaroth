...
builds obfuscate JSON field names, and any field missing from the
new-version block in `rust/istaroth-agd-regen/src/deob.rs` reads as absent
rather than erroring — it produces silently incomplete output (e.g. dropped
...
issue #273: renders as `Missing text (<hash>)` in the corpus). The regen
recovers these via `FALLBACK_REFS` in
`rust/istaroth-agd-regen/src/textmap.rs` — a small list of pinned
older-build commit refs (in `<AGD>`), ordered newest-to-oldest, merged in as
a fallback source when the current TextMap lacks a hash.

1. Regenerate ad hoc into a throwaway directory for both languages (there is
   no partial-regen flag, but a full run takes only seconds; the audit below
   reads just `agd_quest` — dialog-heavy and where regressions concentrate):
...
3. If the new build's count jumped meaningfully above the old corpus's
   ...
   **immediately preceding minor-version build's commit** (from `<AGD>`'s
   ...
   the last commit for that version) to the front of `FALLBACK_REFS`,
   newest-first.
...
4. `AGD_PATH=<AGD> cargo test --manifest-path rust/Cargo.toml` to confirm
   the SEXPRO pronoun-hash resolution and fallback tests in
   `rust/istaroth-agd-regen/tests/agd_integration.rs` still pass — they're
   sensitive to `FALLBACK_REFS` changes.
...
`rust/istaroth-agd-regen/src/firstseen.rs`). Ingesting a new build must
extend the index, or `generate-all` fails on the new build's ids:

1. Append the new version's `(version, commit)` entry to `SNAPSHOTS` in
   `rust/istaroth-agd-regen/src/firstseen_build.rs`. Use the new build's
   snapshot commit in `<AGD>`'s git history; normalize hotfix versions to
   `major.minor`.
2. Rebuild, then run
   `AGD_PATH=<AGD> ./rust/target/fast/istaroth-agd-regen build-first-seen` —
   it scans only the snapshots whose delta files are missing on disk and
   writes `text/first_seen/<version>.json` with the ids not seen in any
   earlier version. The new data file is committed to the `text/` submodule
   as part of the corpus regen commit (step 4); the `SNAPSHOTS` entry is
   committed in the main repo.
